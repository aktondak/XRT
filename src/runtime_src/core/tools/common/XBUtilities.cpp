...
{
  if (inUserDomain)
    return xrt_core::get_total_devices(true).first + xrt_core::get_total_devices(false).first;

  return xrt_core::get_total_devices(false).first;
}

std::shared_ptr<xrt_core::device>
XBUtilities::get_smi_config_device(const std::string& device_bdf, bool inUserDomain)
{
  if (!device_bdf.empty()) {
    auto device = get_device(boost::algorithm::to_lower_copy(device_bdf), inUserDomain);
    if (!device->is_userpf())
      throw std::runtime_error(
        "The specified device is a management PF, which does not support xrt-smi configuration. "
        "Please specify a user device (VF) using --device.");
    return device;
  }

  xrt_core::device_collection device_collection;
  collect_devices(std::set<std::string> {"_all_"}, inUserDomain, device_collection);
  for (const auto& device : device_collection) {
    if (device->is_userpf())
      return device;
  }
  ...
  for (const auto & device : deviceCollection) {
    ...
    if (is_mfg) {
      ...
      try {
        ...
        pt_dev.put("instance", std::to_string(instance));
      }
                            ...
                            std::string& device_bdf)
{
  ...
  if (device_var.defaulted()) {
    if (boost::iequals(device_bdf, "default")) {
      auto device = get_smi_config_device("", is_user_domain);
      if (!device)
        throw std::runtime_error("No devices found.");
      device_bdf = xrt_core::query::pcie_bdf::to_string(
